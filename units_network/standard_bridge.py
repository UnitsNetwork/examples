...
class ERC20BridgeInitiatedEvent:
    ...
    def to_merkle_leaf(self) -> HexBytes:
        ...
        return HexBytes(local_token_bytes + cl_to_bytes + cl_amount_bytes)
...
class StandardBridge(BaseContract):
    ...
    def bridge_erc20(
        ...
        return self.send_transaction(
            "bridgeERC20",
            [token, cl_to, el_amount],
            sender_account,
        )

    def token_ratio(self, token: ChecksumAddress):
        return self.contract.functions.tokenRatios(token).call()
    ...
    def parse_erc20_bridge_initiated(
        self, log: LogReceipt
    ) -> ERC20BridgeInitiatedEvent:
